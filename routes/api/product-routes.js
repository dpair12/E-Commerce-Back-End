...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    if (!productData) {
      res.status(404).json({ message: `Product ID #${id} does not exist.` });
      return;
    }
...
router.delete('/:id', async (req, res) => {
  ...
  .then((categoryToDelete) => {
    ...
    const productName = categoryToDelete.product_name;
    ...
    .then(() => {
      const deleteMessage = `Successfully deleted ${productName} product from database.`;
